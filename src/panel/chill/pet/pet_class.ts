...
import emptyImages from "../../../../media/images/empty.png";
import * as PIXI from "pixi.js";
import { FoodList, FoodClassType } from "../food/foodClass";
import FoodClass from "../food/foodClass";
...
export default class PetClass {
  ...
  public petVersion: number;
  public xpToLevelUp: number;
  ...
  constructor(pet: IPetClass) {
    ...
    this.xpToLevelUp = pet.xpToLevelUp;
    this.petVersion = pet.petVersion;
  }
  ...
  public moveAnimatedSprite({ app }: { app: PIXI.Application }) {
    ...
    if (this.growth !== IPetGrowth.old && this.growth > -1) {
      ...
      this.xpBarFill.x = this.animatedSprite.width / 4 - xpBarWidth / 2;
      this.xpBarFill.y = -this.animatedSprite.height / 4 - 10;
      this.xpBarContainer.x = this.animatedSprite.width / 4 - xpBarWidth / 2;
      // bar is on top of the animated sprite
      this.xpBarContainer.y = -this.animatedSprite.height / 4 - 10;
    }
    if (
  ...
  public async nextGrowth({ app }: { app: PIXI.Application }) {
    if (this.growth < IPetGrowth.old) {
      ...
      if (this.getAnimationPossibility()[this.growth].evolution !== undefined) {
        for (let animation of this.getAnimationPossibility()[this.growth]
          .evolution) {
          time += animation.time;
        }

        this.updateAnimatedSprite({
          animations: this.getAnimationPossibility()[this.growth].evolution,
          app: app,
        });
      }
      this.growth += 1;
      ...
      setTimeout(async () => {
        ...
        if (this.eatingFood.length > 0) {
          await this.updateAnimatedSprite({
            animations: this.getAnimationPossibility()[this.growth].eat,
            app: app,
          });
        } else {
          if (this.growth === IPetGrowth.old) {
            await this.updateAnimatedSprite({
              animations: this.getAnimationPossibility()[this.growth].walk,
              app: app,
            });
            this.animatedSprite.scale.set(1, 1);
          } else {
            await this.updateAnimatedSprite({
              animations: this.getAnimationPossibility()[this.growth].idle,
              app: app,
            });
          }
        }
  ...
  public fillXpBar({ app }: { app: PIXI.Application | undefined }) {
    const xpToBaby = (this.xpToLevelUp / 6) * this.xpMultiplicator;
    const xpToChild = xpToBaby + xpToBaby;
    const xpToTeen = xpToChild + xpToBaby;
    const xpToAdult = xpToTeen + xpToBaby;
    const xpToOld = xpToAdult + xpToBaby;
    console.log("this.xp", this.xp);

    if (this.xp <= xpToBaby) {
      this.xpBarFill.width = (this.xp * this.xpBarContainer.width) / xpToBaby;
    } else if (this.xp <= xpToChild) {
      this.xpBarFill.width =
        ((this.xp - xpToBaby) * this.xpBarContainer.width) / xpToBaby;
    } else if (this.xp <= xpToTeen) {
      this.xpBarFill.width =
        ((this.xp - xpToChild) * this.xpBarContainer.width) / xpToBaby;
    } else if (this.xp <= xpToAdult) {
      this.xpBarFill.width =
        ((this.xp - xpToTeen) * this.xpBarContainer.width) / xpToBaby;
    } else if (this.xp <= xpToOld) {
      this.xpBarFill.width =
        ((this.xp - xpToAdult) * this.xpBarContainer.width) / xpToBaby;
    } else {
      this.xpBarContainer.width = 0;
      this.xpBarFill.width = 0;
    }
    // console.log("this.growth", this.xp, this.xpBarFill.width);
    // console.log("rr", xpToBaby, xpToChild, xpToTeen, xpToAdult, xpToOld);
    ...
    if (
      ...
      this.xpBarFill.width = 0;
      if (app) this.nextGrowth({ app: app });
      if (this.xp >= xpToOld) this.xpBarContainer.width = 0;
    }
  }
  ...
  public async feedPet({
    ...
    //   this.xpBarFill.width = 0;
    // } else {
    // this.xpMultiplicator = 4;

    this.xp += food.xpValue;
    this.fillXpBar({ app: app });
    ...
    //   const xp =
    //     this.xp -
    //     (this.xpToLevelUp / (6 - lastGrowth)) * this.xpMultiplicator;
    //   this.xpBarFill.width =
    //     (xp * this.xpBarContainer.width) /
    //     ((this.xpToLevelUp / (6 - growth)) * this.xpMultiplicator -
    //       (this.xpToLevelUp / (6 - lastGrowth)) * this.xpMultiplicator);
    // }
    //   if (
    //     this.xp ===
    ...
    if (this.state !== petState.eat) {
      await this.updateAnimatedSprite({
        animations: this.getAnimationPossibility()[growth].eat,
        app,
      });
    }
    ...
    const foodAnimation = async (delta: number) => {
      ...
      // let isPetFoodCollided: boolean = isCollided({
      //   object1: this.animatedSprite,
      //   object2: food.foodSprite,
      // });
      // if (isPetFoodCollided) {
      if (
        food.foodSprite.y >=
        ...
        if (this.eatingFood.length === 0) {
          ...
          if (this.growth === IPetGrowth.old) {
            this.updateAnimatedSprite({
              animations: this.getAnimationPossibility()[growth].walk,
              app,
            });
          } else {
            this.updateAnimatedSprite({
              animations: this.getAnimationPossibility()[growth].idle,
              app,
            });
          }
        }
  ...
  public static create(pet: IPet): PetClass {
    ...
    const petClass = new PetClass({
      ...
      growth: pet.growth,
      animatedSprite: PIXI.AnimatedSprite.fromImages([emptyImages]),
      xp: pet.xp,
      xpBarContainer: new PIXI.Graphics(),
      xpBarFill: new PIXI.Graphics(),
      xpMultiplicator: pet.xpMultiplicator,
      xpToLevelUp: pet.xpToLevelUp,
      petVersion: pet.petVersion,
    });
    console.log(
      "petClasssdsdsdsd",
      petClass.getAnimationPossibility()[pet.growth],
      pet.growth
    );
    this.createAnimatedSprite({
      animations: petClass.getAnimationPossibility()[pet.growth].idle,
    }).then((animatedSprite) => {
      petClass.animatedSprite = animatedSprite;
    });
  ...
  public getAnimationPossibility(): AnimationPossibilityObject {
    return {
      [this.petGrowth.egg]: {
        walk:
          this.petVersion === 0
